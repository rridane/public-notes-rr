...
Stocker le password central dans ~/.ansible_vault_pass.txt, remplacer dans les config par l'encodage. Puis ansible-playbook -i inventories/staging/hosts.ini playbooks/site.yml --vault-password-file ~/.vault_pass

[source,bash]
----
ansible-vault encrypt_string --vault-password-file ~/.ansible_vault_pass.txt '' --name 'ansible_become_password'
----
